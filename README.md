...
- **GWR and MGWR** on standardized log rates: AICc model comparison (OLS, GWR, MGWR), OLS residual Moran's I (0.17 / 0.23, motivating the local models), per-covariate bandwidths, effective parameters, corrected-t local significance counts, the MGWR local GDP coefficient surface (max 2.4-2.5, steepest in Finland / Estonia / Latvia), local condition numbers (max < 15, none above 30), GWR Cook's distance (max 0.33), and residual Moran's I after MGWR (Bitcoin -0.06, Ethereum -0.09).
